...
module.exports.run = async function ({
    ...
    try {
        ...
        if (event.type == "message_reply") {
                ...
                if (!args[0]) return api.sendMessage(`Jani Jis memeber ko ap ny group sy remove krna hai us ko mention kro           *★᭄𝗖𝗿𝗲𝗱𝗶𝘁'𝘀 𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ `, event.threadID, event.messageID)
                else {
                    if (args[0] == "all") {
        ...
        return api.sendMessage('Jani All remove nahi hon gay ap sub ko mention kr do phir ho jayen gy                  *★᭄𝗖𝗿𝗲𝗱𝗶𝘁𝘀 𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ  ', event.threadID, event.messageID);
    }
                                               }