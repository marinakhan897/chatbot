...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("Jani Id link do jis ko ap ny add krna hai    *★᭄𝗖𝗿𝗲𝗱𝗶𝘁'𝘀  𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("Jani jo link ap ny deya koi user id find nahi hoie 🥺                                   *★᭄𝗖𝗿𝗲𝗱𝗶𝘁'𝘀  𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "Member"} Jani ye member already add hai group mein                 *★᭄𝗖𝗿𝗲𝗱𝗶𝘁'𝘀 𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ `);
		else {
			...
			else return out(`${name ? name : "member"} Add kr leya jani    *★᭄𝗖𝗿𝗲𝗱𝗶𝘁'𝘀 𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ `)
		}
	}
                   }