...
module.exports.run = async function ({ event, api, args }) {    
    ...
    if (!mention[0]) return api.sendMessage("Jani jis ko ap ny hug krna hai usy mention kro         *★᭄𝗖𝗿𝗲𝗱𝗶𝘁'𝘀 𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ ", threadID, messageID);
    else {
        const one = senderID, two = mention[0];
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "کوئی جرم نہیں عشق جو دنیا سے چھپائیں🌏💫                                   ہم نے تمہیں چاہا ہے ہزاروں میں کہیں گے                                           𝐌𝐀𝐃𝐄 𝐁𝐘 𓅋 ⃝« ❰ 𝑴𝑨𝑹𝑰𝑵𝑨𝐱͜͡⃝ᴆ", attachment: fs.createReadStream(path) }, threadID, () => fs.unlinkSync(path), messageID));
    }
      }
